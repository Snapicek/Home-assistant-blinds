...
- **Manual override** — dedicated switch holds the current position for a configurable number of minutes, then auto-clears; no external `timer` helper needed. Manually moving either cover (e.g. a physical remote/wall switch) is detected immediately and always wins over automation: a manual move is latched synchronously so an in-flight automation command is aborted rather than overwriting you, the paired cover is mirrored to the matching calibrated position, and the override is engaged (and extended on continued activity). When the hold expires the tracked state is re-seeded from the covers' real position before automation resumes. A configurable **max travel time** distinguishes a slow motor still settling from a genuine manual move.
